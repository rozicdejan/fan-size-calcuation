...
st.title("🌀 Electrical Cabinet Climate Tool (Rittal DB)")
...
if indoor_outdoor == "Outdoor":
    st.markdown("### Cabinet Geometry (for solar & heater)")
    ...
    absorptivity = st.select_slider("Paint absorptivity (≈ RAL color)", options=[0.3, 0.5, 0.7, 0.9], value=0.7)
...
max_inside = st.number_input("Max allowed cabinet air temp (°C)", value=40.0)
...
if dew_point >= ambient:
    st.error(f"⚠️ Condensation risk likely. Suggest heater ≈ {heater_power:.0f} W")
elif dew_point >= ambient - 2:
    st.warning("⚠️ Dew point close to ambient. Monitor humidity / consider heater.")
else:
    st.success("✅ Low condensation risk.")

delta_t = max_inside - ambient
total_load = total_power + solar_power
st.markdown(f"### Heat Load Summary")
st.write(f"- Devices: **{total_power:.1f} W**")
if solar_power > 0: st.write(f"- Solar: **{solar_power:.1f} W**")
st.write(f"**Total load: {total_load:.1f} W**")
...
if total_load > 0 and delta_t > 0:
    ...
    if rec:
        ...
        st.success(
            f"✅ **{rec['n']} × {rec['model']}**  \n"
            f"- Total airflow ≈ **{rec['flow_total']} m³/h**  \n"
            f"- Estimated fan power ≈ **{rec['power_total']:.0f} W**"
        )
        if fan_detail:
            extra_lines = []
            for key, label in {
                ...
                if key in fan_detail:
                    extra_lines.append(f"- {label}: **{fan_detail[key]}**")
            if extra_lines: st.markdown("\n".join(extra_lines))
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.download_button("📊 Download Excel", export_excel(export_data),
                           file_name="fan_sizing.xlsx")
    with c2:
        st.download_button("📄 Download PDF",
                           export_pdf(export_data, redundancy_info, airflow_fig, redundancy_fig, pq_fig),
                           file_name="fan_sizing.pdf", mime="application/pdf")
else:
